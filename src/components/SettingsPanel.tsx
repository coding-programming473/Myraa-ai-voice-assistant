...
export function SettingsPanel({ isOpen, onClose, settings, onChange, themeColor }: SettingsPanelProps) {
  ...
  const [apiKeyMessage, setApiKeyMessage] = useState<{ ok: boolean; text: string } | null>(null);
  ...
  useEffect(() => {
    ...
    void fetch("/api/config", { cache: "no-store" })
      .then((res) => res.json())
      .then((data) => setApiKeyConfigured(Boolean(data.hasApiKey)))
  ...
  const saveApiKey = async () => {
    ...
    try {
      ...
      setApiKeyValue("");
      setApiKeyMessage({ ok: true, text: "API key verified and saved. Restart the voice link with Awake Myraa." });
    } catch (error) {
      setApiKeyMessage({
        ok: false,
        text: error instanceof Error ? error.message : "The API key could not be saved.",
      });
    } finally {
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 rounded-xl border border-white/10 bg-white/[0.03] p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <label className="block text-[10px] font-mono tracking-wider text-slate-200 uppercase">
                          Character Shine
                        </label>
                    ...
                    <input
                      ...
                      max={100}
                      step={1}
                      value={settings.characterShine}
                      onChange={(e) => onChange({ characterShine: Number(e.target.value) })}
                      className="w-full accent-cyan-500 cursor-pointer"
                    />
                    <div className="flex justify-between text-[8px] font-mono uppercase tracking-wider text-slate-600">
                      <span>Matte</span>
                      <span>Balanced</span>
                      <span>Glossy</span>
                    </div>
                  ...
                  <div className="rounded-xl border border-cyan-400/10 bg-cyan-400/[0.04] p-3 text-[9px] font-mono leading-relaxed text-slate-400">
                  ...
                  <div className="space-y-3 rounded-xl border border-white/10 bg-white/[0.03] p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="text-[10px] font-bold font-mono text-slate-200">GEMINI API KEY</div>
                          <div className="text-[8px] uppercase font-mono text-slate-500">
                            Stored securely by the local MYRAA backend
                          </div>
                        </div>
                      </div>
                      <span className={`shrink-0 rounded-full border px-2 py-1 text-[8px] font-mono uppercase ${
                        apiKeyConfigured
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        autoComplete="off"
                        spellCheck={false}
                        placeholder={apiKeyConfigured ? "Enter a new key to replace it" : "Paste Gemini API key"}
                        aria-label="Gemini API key"
                        className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-white font-mono outline-none transition focus:border-indigo-400/50"
                      />
                      <button
                        type="button"
                        onClick={() => void saveApiKey()}
                        disabled={apiKeySaving || !apiKeyValue.trim()}
                        className="flex min-w-20 items-center justify-center rounded-xl border border-indigo-400/25 bg-indigo-500/15 px-3 py-2 text-[9px] font-bold font-mono uppercase text-indigo-200 transition hover:bg-indigo-500/25 disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        {apiKeySaving ? <Loader2 size={13} className="animate-spin" /> : apiKeyConfigured ? "Replace" : "Save"}
                      </button>
                    </div>
                    {apiKeyMessage && (
                      <div className={`rounded-lg border px-3 py-2 text-[9px] font-mono ${
                        apiKeyMessage.ok
